...
static function recurrences($value, $params, $data=array()) {
  ...
  foreach ($values as $key=>$val) {
    if ($val > $begin) $result[$key] = sys_date("{t}m/d/Y{/t}", $val);
  ...
  if ($length>20) {
	...
	if (strpos($str,"\n&gt;")) {
	  $rep = "<span><p><br><a href='#' onclick='showhide_inline(this.parentNode.parentNode.lastChild); return false;'>".
		"&gt; {t}Show quoted text{/t}</a><br><div style='display:none;'>\\1</div></span>";
...
static function htmlfield($text,$allow_pics=true,$allow_iframe=false) {
  ...
  if (strpos($result, "<blockquote>")) {
	...
	$with = array(
	  "<span><a href='#' onclick='showhide_inline(this.parentNode.lastChild); return false;'>&gt; {t}Show quoted text{/t}</a><br><div style='display:none;'><blockquote>",
	  "</blockquote></div></span>",
	);
    $result = str_replace($replace, $with, $result);
  }
  if ($add_image_switch) return " <span><a href='#' onclick='display_images(); hide(this.parentNode); return false;'>&gt;&gt; {t}Load external images{/t}</a><br/><br/></span>".$result;
  return $result;
}
...
static function replacechecked($checked) {
  if ($checked=="") return "";
  if ($checked=="1") return "{t}yes{/t}";
  return "{t}no{/t}";
}

static function receipt($value) {
  if ($value=="{t}yes{/t}") return " "; else return "";
}
...
static function localdateformat($args,$format) {
  ...
  $short_months = array("","{t}Jan{/t}","{t}Feb{/t}","{t}Mar{/t}","{t}Apr{/t}","{t}May{/t}","{t}Jun{/t}",
  				  "{t}Jul{/t}","{t}Aug{/t}","{t}Sep{/t}","{t}Oct{/t}","{t}Nov{/t}","{t}Dec{/t}");
  $days = array("{t}Sunday{/t}","{t}Monday{/t}","{t}Tuesday{/t}","{t}Wednesday{/t}","{t}Thursday{/t}","{t}Friday{/t}","{t}Saturday{/t}");
...
static function dateformat($args,$args2=null) {
  ...
  if (is_array($args2)) {
	...
	if (empty($args2[1])) $args2[1] = "{t}m/d/Y{/t}";
...
static function forwardmessage($value,$args,$data) {
  $value = trim(str_replace("---\r\n".SMTP_FOOTER,"",$value));
  $val = "-------- {t}Original message{/t} --------\n";
  if (!empty($data["subject"]["data"][0])) $val .= "{t}Subject{/t}: ".$data["subject"]["data"][0]."\n";
  if (!empty($data["created"])) $val .= "{t}Date{/t}: ".self::dateformat($data["created"],array("","{t}m/d/Y{/t}"))."\n";
  if (!empty($data["efrom"]["data"][0])) $val .= "{t}From{/t}: ".$data["efrom"]["data"][0]."\n";
  if (!empty($data["eto"]["data"][0])) $val .= "{t}To{/t}: ".$data["eto"]["data"][0]."\n";
  if (!empty($data["cc"]["data"][0])) $val .= "{t}Cc{/t}: ".$data["cc"]["data"][0]."\n";
  return "\n\n\n".$val."\n".$value;
}
...
static function replysubject($value) {
  if (isset($_REQUEST["return_receipt"])) return "{t}Read{/t}: ".$value;
  return "Re: ".$value;
}
...
static function duration($val, $params) {
  ...
  $units = array("sec"=>"","min"=>"{t}min{/t}","hours"=>"{t}hours{/t}","days"=>"{t}days{/t}","weeks"=>"{t}weeks{/t}","months"=>"{t}months{/t}","years"=>"{t}years{/t}");
...
static function decode_subject($input) {
  ...
  if ($input!="" and strpos($input,"=?")!==false) {
	...
	$charset = "";
	$match = array();
   	while (preg_match('/(=\?([^?]+)\?(q|b)\?([^?]*)\?=)/i', $input, $match)) {
      $text = $match[4];
	  $charset = strtolower($match[2]);
      switch (strtolower($match[3])) { // encoding
   	    case "b": $text = base64_decode($text); break;
   	    case "q":
          $text = str_replace("_"," ",$text);
		  $match2 = array();
       	  preg_match_all("/=([a-f0-9]{2})/i",$text,$match2);
 	      foreach ($match2[1] as $value) $text = str_replace('='.$value, chr(hexdec($value)), $text);
          break;
   	  }
      $input = str_replace($match[1],$text,$input);
	  if ($i>4) break;
	  $i++;
    }
