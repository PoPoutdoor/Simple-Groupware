...
function asset_process_session_request() {
  ...
  if (!empty($_REQUEST["iframe"]) and isset($_REQUEST["session_remove_request"])) $t["sqllimit"] = array(); // offline reading
...
// /Workspace/<node>/<node>/...
// node: folder name (first), !ftype (in path), ^anchor (first), * (all)
...
function sys_get_xml($file, $data = array()) {
  ...
  $xml = trans(preg_replace("|<!--.*?-->|msi","",$xml));
  try {
