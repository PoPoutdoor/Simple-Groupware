  ...
  if (count($data)==0) {
    ...
    foreach ($data as $asset) {
	  ...
	  if ($recurrence) {
		...
		if ($repeatuntil != 0) $output .= ";UNTIL=".sys_date("Ymd",$repeatuntil)."T235900";
		$output .= "\n";
	    if (count($excludes)>0) {
...
private static function _vcard_data() {
  ...
  if (count($data)==0) {
	$output .= "BEGIN:VCARD\nVERSION:3.0\n";
	$output .= "FN:{t}No entries found.{/t}\nN:{t}No entries found.{/t};---\nEND:VCARD\n";
  } else {
    $url = self::_url_folder();
    foreach ($data as $asset) {
      $output .= "BEGIN:VCARD\nVERSION:3.0\n";
...
private static function _xml($isfeed) {
  ...
  if ($isfeed) {
    ...
    $url = q(self::_url()."folder=".rawurlencode($t["folder"])."&view=".$tview);
	$output .= "<rss version=\"2.0\">\n<channel>\n";
	$output .= "<title>".q($GLOBALS["sel_folder"]["ftitle"]." - ".$view." - ".APP_TITLE)."</title>\n";
    ...
    $output .= "<title>".q(APP_TITLE)."</title>\n";
	$output .= "<width>-15</width>\n";
    $output .= "</image>\n";
	
    $output .= "<item>\n";
    $output .= "<title>".q($GLOBALS["sel_folder"]["ftitle"]." - ".$view)."</title>\n";
    $output .= "<link>".$url."</link>\n";
    $output .= "<pubDate>".sys_date("r")."</pubDate>\n";
    ...
    foreach (self::_build_data(false,array(),true) as $asset) {
	  ...
	  foreach ($asset as $akey=>$aval) {
	    if ($aval["name"]) $output .= "<".$akey.">".q($aval["filter"])."</".$akey.">\n";
	  }
	  $output .= "</asset>\n";
    }
    $output .= "</".$t["att"]["NAME"].">\n";
...
private static function _sss_editor($output) {
  echo '  
    ...
    <script src="ext/lib/simple_spreadsheet/translations/'.LANG.'.js" type="text/javascript"></script>
	<script src="ext/lib/simple_spreadsheet/spreadsheet.js" type="text/javascript"></script>
	</head>
	<body>
	<div id="data" class="data"></div>
	<div id="source">
	<textarea id="code" wrap="off">'.q($output).'</textarea>
