...
if (preg_match("!desktop\.ini\$|folder\.jpg|folder.gif|target\.lnk\$|\.dll\$|thumbs\.db\$!i", $_SERVER["REQUEST_URI"])) {
...
$dav_folder = "/sgdav";
if (($pos = strpos($_SERVER["REQUEST_URI"], "webdav.php"))) {
  $dav_folder = substr($_SERVER["REQUEST_URI"], 0, $pos+10);
}
$dav_folder_q = preg_quote($dav_folder);

if ($_SERVER["REQUEST_METHOD"]=="GET") {
  webdav_forward("^".$dav_folder_q."/(.+)/([0-9]+)(_[^_]+)?_([0-9]+)__.+", "download.php", "view=display&dispo=noinline&folder=/%s/&item=%s&field=%s&subitem=%s");
  
  webdav_forward("^".$dav_folder_q."/(.+)/index\.html", "index.php", "view=details&export=html&folder=/%s/");
  
  webdav_forward("^".$dav_folder_q."/(.+)/index\.xls", "index.php", "view=details&export=calc&folder=/%s/");

  webdav_forward("^".$dav_folder_q."/.+/.+", "download.php", "view=display&dispo=noinline&item=session");
}

if ($_SERVER["REQUEST_METHOD"]=="PUT") {
  webdav_forward("^".$dav_folder_q."/(.+)/([0-9]+)(_[^_]+)?_([0-9]+)__(.+)", "upload.php", "view=display&dispo=noinline&folder=/%s/&item=%s&field=%s&subitem=%s&filename=%s&action=PUT");

  webdav_forward("^".$dav_folder_q."/.+/.+", "upload.php", "item=session&action=PUT");
}

if ($_SERVER["REQUEST_METHOD"]=="MOVE") {
  // destination: e.g. http://localhost/sgdav/Workspace/Demo/Files/6101_0__bla.jpg
  webdav_forward("\.tmp\$", "upload.php", "view=display&dispo=noinline&folder=/%s/&item=%s&field=%s&subitem=%s&action=MOVE&filename=".$_SERVER["REQUEST_URI"], $dav_folder_q."/(.+)/([0-9]+)(_[^_]+)?_([0-9]+)__.+");

  webdav_forward("\.tmp\$", "upload.php", "view=display&dispo=noinline&item=session&action=MOVE&filename=".$_SERVER["REQUEST_URI"], $dav_folder_q."/.+/.+");
}
...
// TODO 2 path lookup cache ? protect get file-xy floods ?
...
<D:href>'.q(dirname($data["path"])."/".rawurlencode(basename($data["path"]))).'</D:href>
<D:propstat>
<D:prop>'.
...
<lp1:displayname>'.q(basename($data["path"])).'</lp1:displayname>'.
...
function webdav_propfind($path,$dav_folder) {
  ...
  $id = 0;
  $file = "";
  if (in_array(basename($path),array("index.html","index.xls"))) {
...
function webdav_build_files($path,$cfolder) {
  $out = "";
  $tview = "details";
  foreach (array("index.html","index.xls") as $val) {
