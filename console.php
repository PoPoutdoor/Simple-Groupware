...
require("index.php");
if (!sys_is_super_admin($_SESSION["username"])) sys_die(t("Not allowed. Please log in as super administrator."));
...
if (!empty($_REQUEST["code"])) {
  if (!sys_validate_token()) sys_die(t("Invalid security token"));
  $code = $_REQUEST["code"];
}
...
if ($code!="") {
  $content .= "<br/>&nbsp;".t("%s secs", number_format(microtime(true)-$start, 4));
  $content .= ", ".t("%s M memory usage", number_format(memory_get_peak_usage(true)/1048576, 2));
}
