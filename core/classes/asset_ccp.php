...
static function paste_items($folder, $ccp_data) {
  ...
  if ($cut and $tname!=$ccp_data["tname"]) {
	...
	if (!$allowed) exit("{t}Operation not allowed. (Data loss protection){/t}");
...
private static function _validate_value($tname, $field, $data, $key) {
  ...
  if (isset($field["REQUIRED"]) and empty($data)) {
    $error[$key][] = "{t}missing field{/t} [".$key_name."]";
...
private static function _move($ccp, $folder, $sgsml) {
  ...
  if (is_array($rows) and count($rows) < count($ccp["items"])) exit("{t}Item(s) not found or access denied.{/t}");
  ...
  foreach ($rows as $row) {
	...
	if (!is_numeric($folder)) {
	  ...
	  if ($error_sql!="") $messages[] = "{t}SQL failed.{/t} ".$error_sql;
	  ...
	  if (!empty($sgsml->att["TRIGGER_EDIT"]) and empty($sgsml->att["DISABLE_TRIGGER_CCP"])) {
		...
		if ($return!="") $messages[] = "{t}Trigger failed{/t}: ".$return;
	  }
