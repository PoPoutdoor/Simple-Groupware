...
function gen_date(dt_datetime) {
	var date_str = caller.date_format;
...
function gen_time(dt_datetime) {
	var time_str = caller.time_format;
...
function prs_date(str_date) {
	var reg = new RegExp(caller.date_format.replace("d","(\\d{1,2})").replace("m","\\d{1,2}").replace("Y","\\d{4}"));
	reg.exec(str_date);
	var day = RegExp.$1;
	var reg = new RegExp(caller.date_format.replace("d","\\d{1,2}").replace("m","(\\d{1,2})").replace("Y","\\d{4}"));
	reg.exec(str_date);
...
function prs_time(str_time, dt_date) {
	...
	var reg = new RegExp(caller.time_format.replace(/G|H|g|i/g,"(\\d{1,2})").replace("a","(am|pm)"));
	reg.exec(str_time);
	var hours = RegExp.$1;
...
var caller = top.calendars[num_id];
if (caller && caller.year_scroll) {
...
dt_firstday.setDate(1 - (7 + dt_firstday.getDay() - caller.NUM_WEEKSTART) % 7);
function set_datetime(n_datetime, b_close) {
	...
	if (b_close) {
		...
		if (n_datetime) {
		  caller.target.value = (document.cal ? gen_tsmp(dt_datetime) : gen_date(dt_datetime));
		  if (caller.callback) top.calendar_changed();
		}
	} else caller.popup_open(dt_datetime.valueOf());
}
</script>
</head>
...
<script language="JavaScript">
document.write(
'<td>'+(caller&&caller.year_scroll?'<a href="javascript:set_datetime('+dt_prev_year.valueOf()+')">&lt;</a>&nbsp;':'')+'<a href="javascript:set_datetime('+dt_prev_month.valueOf()+')">&lt;&lt;</a></td>'+
'<td align="center" width="100%"><b><font color="#ffffff">'+ARR_MONTHS[dt_current.getMonth()]+' '+dt_current.getFullYear() + '</font></b></td>'+
'<td><a href="javascript:set_datetime('+dt_next_month.valueOf()+')">&gt;&gt;</a>'+(caller && caller.year_scroll?'&nbsp;<a href="javascript:set_datetime('+dt_next_year.valueOf()+')">&gt;</a>':'')+'</td>'
);
</script>
</tr>
</table></td></tr>
...
<script language="JavaScript">
for (var n=0; n<7; n++)
	document.write('<td bgcolor="#87cefa" align="center"><font color="#ffffff">'+ARR_WEEKDAYS[(caller.NUM_WEEKSTART+n)%7]+'</font></td>');
document.write('</tr>');
var dt_current_day = new Date(dt_firstday),
	dt_selected = caller.dt_selected;
...
<script>
document.write('<div id="footer"><a href="javascript:set_datetime(new Date().valueOf(),true);">'+caller.str_today+'</a> - <a href="javascript:set_datetime(\'\',true);">'+caller.str_close+'</a></div>');
</script>
</td></tr></table>
<script>
